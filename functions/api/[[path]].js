...
function splitMemoToPieces(input) {
  ...
  // Single-line but long: split into short sentence-like pieces.
  const pieces = [];
  let buf = '';
...
function pickKeyPieces(pieces, maxPieces) {
...
function decideKeyPieceBudget(totalMemoChars, totalPieces) {
  // Higher input volume => fewer key pieces per task.
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (parts.length === 2 && parts[0] === 'gpt' && parts[1] === 'report-from-timeline' && request.method === 'POST') {
      ...
      // Build compact input: dedupe and pick key points from memos.
