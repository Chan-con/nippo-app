...
class SupabaseTaskManager {
  ...
  async saveGoalStock(goals, userId) {
    if (!userId) throw new Error('userId is required');
    await this._setDoc(userId, 'goal_stock', 'default', { goals: Array.isArray(goals) ? goals : [], updatedAt: new Date().toISOString() });
    return true;
  }
  ...
  async saveTaskStock(tasks, userId) {
    if (!userId) throw new Error('userId is required');
    await this._setDoc(userId, 'task_stock', 'default', { tasks: Array.isArray(tasks) ? tasks : [], updatedAt: new Date().toISOString() });
    return true;
  }
  ...
  async saveTagStock(tags, userId) {
    if (!userId) throw new Error('userId is required');
    await this._setDoc(userId, 'tag_stock', 'default', { tags: Array.isArray(tags) ? tags : [], updatedAt: new Date().toISOString() });
    return true;
  }
...
function createSupabaseAuthMiddleware({ supabaseUrl, anonKey }) {
  if (!supabaseUrl) throw new Error('SUPABASE_URL is required');
